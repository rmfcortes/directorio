  ...
  guardarHorario() {
    ...
    if (this.datos.corrido) {
      if (!this.datos.cierreComida || !this.datos.reapertura) {
        this.presentToast('Si el horario no es corrido. La hora de cierre y reapertura para comida son obligatorios');
        return;
      }
      if (this.datos.cierreComida > this.datos.reapertura) {
      ...
      if (this.datos.cierreComida > this.datos.horaCierre || this.datos.reapertura < this.datos.horaApertura) {
