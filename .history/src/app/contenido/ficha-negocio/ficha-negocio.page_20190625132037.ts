...
export class FichaNegocioPage implements OnInit {
  ...
  async getDatos() {
    this.activatedRoute.params.subscribe(async (data) => {
      ...
      if (this.uid && this.negocio.hasProductos) {
        ...
        this.productos = await this.hasCart([...this.productos]);
        this.productosDestacados = await this.pasilloService.updateProductsDestacadosQty([...this.productosDestacados]);
  ...
  async hasCart(prods) {
    ...
    const prodsAndCart: any = await this.pasilloService.getCart(this.uid, this.id, prods);
