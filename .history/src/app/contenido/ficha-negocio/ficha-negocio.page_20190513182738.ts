...
export class FichaNegocioPage implements OnInit {
  ...
  isOpen() {
    ...
    if (this.horario[index].reapertura) {
      reapertura = this.horario[index].reapertura.split(':');
      const horaC = reapertura[0];
      const minC = reapertura[1];
      aCTime = fecha.setHours(parseInt(horaC, 10), parseInt(minC, 10));
    }

    if (this.horario[index].cierreComida) {
      cierreComida = this.horario[index].cierreComida.split(':');
