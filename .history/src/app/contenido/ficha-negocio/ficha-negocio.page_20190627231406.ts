...
import { Pregunta, Pasillo, Producto, Calificacion } from 'src/app/interfaces/negocio.interface';
...
export class FichaNegocioPage implements OnInit {
  ...
  async presentCalificar() {
    ...
    const modal = await this.modalController.create({
      ...
      componentProps: { tipo: calificacion }
    });

    modal.onDidDismiss().then(async (comentario) => {
      if (comentario) {
        this.opiniones.unshift(comentario);
      }
    });
    return await modal.present();
  }
