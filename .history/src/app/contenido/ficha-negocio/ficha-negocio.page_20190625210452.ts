...
export class FichaNegocioPage implements OnInit {
    ...
    this.activatedRoute.params.subscribe(async (data) => {
      ...
      if (this.uid && this.negocio.hasProductos) {
        ...
        this.productos.forEach(async (p, i) => {
          ...
          if (p.seguidores) {
            console.log(p.seguidores);
            p.seguidores = p.filter(seguidor => seguidor === this.uid);
          }
  ...
  async getPendientes() {
    return new Promise(async (resolve, reject) => {
      ...
      if (pendiente) {
        ...
        if (type === 'producto') {
          await this.addProduct(this.productos[pos.pri].productos[pos.sec], pos.pri, pos.sec);
  ...
  async getFavoritos(prods) {
    ...
    let result = prods;
    const favs = await this.pasilloService.getAllProductosFavoritos(this.id);
    if (favs) {
      this.favs = Object.keys(favs);
      result = await this.isProdFavorito(prods);
    }
    return result;
  }

  isProdFavorito(productos) {
  ...
  async addProduct(producto, i, y) {
    return new Promise(async (resolve, reject) => {
      const resp = await this.pasilloService.addProduct(producto, i, this.id, y);
      if (resp) { this.productos[i].productos[y].cart = true; }
      resolve();
    });
  }

  plusData(data) {
    this.plusProduct(data.producto, data.i, data.y);
  }
  ...
  minusData(data) {
    this.minusProduct(data.producto, data.i, data.y);
  }
