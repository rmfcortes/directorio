...
export class FichaNegocioPage implements OnInit {
  ...
  async getDatos() {
    this.activatedRoute.params.subscribe(async (data) => {
      ...
      if (this.uid && this.negocio.hasProductos) {
        ...
        this.productosDestacados = await this.pasilloService.updateProductsQty([...this.productosDestacados], false);
        this.productos.forEach(async (p, i) => {
          this.productos[i].productos = await this.pasilloService.updateProductsQty(Object.values(p.productos), false);
        });
      }
      this.infoReady = true;
    });
  ...
  async addProduct(producto, i, y?) {
    console.log(y);
    await this.pasilloService.addProduct(producto, i, y, this.id);
    if (y) { this.productos[i].productos[y].cart = true; } else {
      this.productosDestacados[i].cart = true;
    }
  }
