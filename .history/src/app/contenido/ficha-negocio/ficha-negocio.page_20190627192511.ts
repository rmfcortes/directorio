...
export class FichaNegocioPage implements OnInit {
  ...
  async getDatos() {
    this.activatedRoute.params.subscribe(async (data) => {
      ...
      const load = await this.variableService.getLoaded();
      if (load) {
        this.setInfo();
        return;
      }
      ...
      if (this.negocio.hasPreguntas) { await this.getPreguntas(); }
      if (this.negocio.hasOfertas) { await this.hasOfer(); }
      if (this.negocio.hasValoraciones) { await this.hasValora(); }
  ...
  async setInfo() {
    const info: any = this.variableService.getInfo();
    await this.pasilloService.getCart(this.id);
    this.productos = info.productos;
    this.negocio = info.negocio;
    this.sucursales = info.sucursales;
    this.variableService.setLoaded(false);
    this.variableService.setInfo(null);
    this.infoReady = true;
    this.prodsReady = true;
    this.getPendientes();
    return;
  ...
  cargaInfo() {
    ...
    this.variableService.setLoaded(true);
    this.variableService.setInfo(info);
    return;
  }
  ...
  async preguntar() {
    ...
    this.preguntas.unshift(this.duda);
    this.usuarioService.publicarPregunta(this.duda, this.negocio.categoria, this.uid);
    return;
  }
