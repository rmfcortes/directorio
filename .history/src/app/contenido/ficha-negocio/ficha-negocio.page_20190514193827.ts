...
export class FichaNegocioPage implements OnInit {
  ...
  getValoraciones() {
    const valorSub = this.negocioService.getValoraciones(this.id).subscribe( (data: any) => {
      this.valoraciones.cantidad = data.length;
      this.valoraciones.comentarios = data;
  ...
  getPromedio() {
    let puntaje = 0;
    this.valoraciones.comentarios.forEach( opinion => {
      puntaje += parseInt(opinion.puntos, 10);
    });
    this.valoraciones.promedio = puntaje / this.valoraciones.cantidad;
  }
