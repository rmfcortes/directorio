...
export class FichaNegocioPage implements OnInit {
  ...
  async isFavorito() {
    try {
      const resp = await this.usuarioService.getFavorito(this.negocio.id, 'negocios', this.negocio.categoria);
      if (resp) {
        this.guardado = true;
      } else {
        this.guardado = false;
  ...
  async agregarFavorito() {
    ...
    try {
      await this.usuarioService.guardarFavorito(favorito, 'negocios');
  ...
  async borrarFavorito() {
    try {
      await this.usuarioService.borrarFavorito(this.negocio.id, 'negocios', this.negocio.categoria);
      this.guardado = false;
    } catch (error) {
      console.log(error);
    }
