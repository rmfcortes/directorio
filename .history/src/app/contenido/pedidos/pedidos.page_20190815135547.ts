...
export class PedidosPage implements OnInit {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      this.statuSub = state.subscribe((value: Pedido) => {
        ...
        if (value.paso1 && !value.paso2) {
          this.entrega = value.paso1 + value.preparacion;
        }
        resolve();
      });
    });
  ...
  async muestraDetalles() {
    const canasta = await this.usuarioService.getCarrito(this.uid, this.pedido.id);
    const count = await this.usuarioService.getCuenta(this.uid, this.pedido.id);
    const modal = await this.modalCtrl.create({
      component: DetallePedidoComponent,
      componentProps: {
        direccion: this.status.direccion.direccion,
        cart: canasta,
        cuenta: count
      }
    });
  ...
  llamarNegocio() {
    this.callNumber.callNumber(this.status.negocio.telefono, true)
