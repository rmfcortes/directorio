...
export class FichaBazarPage implements OnInit, OnDestroy {
  ...
  async getFavorito(clasificado) {
    const resp = await this.usuarioService.getFavorito(this.articulo.id, 'clasificados', clasificado);
    if (resp) {
  ...
  async favorito() {
    try {
      ...
      await this.usuarioService.guardarFavorito(fav, 'clasificados');
      this.isFavorito = true;
    } catch (error) {
      console.log(error);
    }
  ...
  async borrarFavorito() {
    try {
      ...
      await this.usuarioService.borrarFavorito(this.articulo.id, 'clasificados', clasificado);
      this.isFavorito = false;
    } catch (error) {
      console.log(error);
    }
