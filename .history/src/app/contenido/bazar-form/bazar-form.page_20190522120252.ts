...
export class BazarFormPage implements OnInit {
  ...
  async publicar() {
    ...
    try {
      ...
      const urls = await this.anuncioService.publicarFotosBazar(this.fotosBase64, this.uid, id);
      this.producto.url = urls;
      const resp = await this.anuncioService.publicarBazar(this.producto, this.uid);
      if (resp) {
