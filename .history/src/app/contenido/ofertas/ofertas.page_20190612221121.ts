...
export class OfertasPage implements OnInit {
    ...
    const s = this.ofertaService.getOfertas(this.batch + 1).valueChanges()
      .subscribe((ofertas: any) => {
        ...
        if (ofertas) {
          ...
          this.ofertas = ofertas.reverse().shift();
  ...
  // async getOfertas() {
  //   const s = this.ofertaService.getOfertas(this.batch + 1, this.ofertasQt, this.step).valueChanges()
  //     .subscribe(ofertas => {
  //       s.unsubscribe();
  //       if (ofertas) {
