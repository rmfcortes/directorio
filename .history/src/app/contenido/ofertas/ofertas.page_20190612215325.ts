...
export class OfertasPage implements OnInit {
  ...
  async getOfertas() {
    const s = this.ofertaService.getOfertas(this.batch, this.ofertasQt, this.step).valueChanges()
      .subscribe(ofertas => {
        s.unsubscribe();
        if (ofertas) {
          this.ofertas = ofertas.reverse();
          this.oldArray = [...this.ofertas];
    ...
    const s = this.ofertaService.getOfertas(this.batch, this.ofertasQt, this.step).valueChanges()
      .subscribe(ofertas => {
        ...
        if (ofertas) {
          ...
          this.ofertas = this.oldArray.concat(ofertas);
          console.log(this.ofertas);
          this.oldArray = [...ofertas];
        }
      });
