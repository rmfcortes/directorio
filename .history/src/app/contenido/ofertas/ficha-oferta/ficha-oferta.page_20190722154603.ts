...
export class FichaOfertaPage implements OnInit {
  ...
  async presentNegocio() {
    const negocio: VistaPreviaNegocio = await this.ofertaService.getInfoNegocio(this.oferta.idNegocio);
  ...
  regresar() {
    ...
    this.modalController.dismiss({info: respuesta});
  }
