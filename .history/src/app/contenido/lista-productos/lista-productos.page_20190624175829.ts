...
export class ListaProductosPage {
  ...
  async getDatos() {
    this.activatedRoute.params.subscribe(async (data) => {
      ...
      const favs = await this.isFavorito(cart);
      this.productos = favs;
      this.infoReady = true;
  ...
  getCart(prods) {
    ...
    if (cart) {
      ...
      const cartByCat: any = await this.pasilloService.getCartCategoria(this.uid, this.id, this.categoria);
      console.log(cartByCat);
      if (cartByCat) {
        this.localProdsCart = cartByCat;
  ...
  updateProductsQty(prods) {
    ...
    this.localProdsCart.forEach((prod, i) => {
      ...
      if (index >= 0) {
        ...
        this.localProdsCart.splice(i, 1);
      }
    });
    return prods;
  }

  updateProductsQtyLoad(prods) {
    console.log(prods);
    prods.forEach((prod, i) => {
      const index = this.productos.findIndex(p => p.id === prod.id);
      if (index >= 0) {
        this.productos[index].cantidad = prod.cantidad;
        this.productos[index].index = prod.index;
        this.productos[index].cart = true;
        prods.splice(i, 1);
      }
    });
  }
