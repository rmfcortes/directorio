...
export class FichaNegocioPage {
  ...
  async muestraProducto(prod, pas, i, y) {
  ...
  async muestraPasillo(type) {
    ...
    modal.onDidDismiss().then((resp: any) => {
      ...
      const indexFav = this.productos.findIndex(p => p.nombre === type);
      const index = this.productos.findIndex(p => p.nombre === 'Favoritos');
      this.productos[index].productos = [];
      console.log(index);
      console.log(indexFav);
      console.log(resp.data.prods);
      if (indexFav >= 0) {
