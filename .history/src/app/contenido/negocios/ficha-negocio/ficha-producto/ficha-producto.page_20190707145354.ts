...
export class FichaProductoPage implements OnInit {
  ...
  async agregarProductoFavorito() {
    if (!this.producto.guardado) {
      try {
        ...
        await this.pasilloService.addProductoFav(this.producto, this.id);
  ...
  async plusProduct(producto) {
    this.producto.cantidad += 1;
    this.pasilloService.plusProduct(producto, this.id);
  }

  async minusProduct(producto) {
    this.producto.cantidad -= 1;
    if (this.producto.cantidad === 0) {
      this.producto.cart = false;
    }
    this.pasilloService.restProduct(producto, this.producto.cantidad, this.id);
  }
