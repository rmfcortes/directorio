...
export class FichaNegocioPage {
  ...
  async getInfoProdsBloque() {
    return new Promise(async (resolve, reject) => {
      let prod: Pasillo[] = await this.negocioService.getProductos(this.id, this.batch + 1);
      ...
      this.productos = await this.evaluaCantBloque(prod);
  ...
  async evaluaCantBloque(data): Promise<Pasillo[]> {
    return new Promise(async (resolve, reject) => {
      for (const iterator of data) {
        iterator.productos = await this.pasilloService.updateProductsQty(iterator.productos);
      }
      resolve(data);
    });
  }
