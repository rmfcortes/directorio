...
export class FichaNegocioPage {
  ...
  async addProduct(producto: Producto, pasillo: string, i: number, y: number) {
    this.prodsPreviewInCart.push(producto);
    console.log(this.prodsPreviewInCart);
    if (!this.uid) {
  ...
  async minusProduct(producto, i, y) {
    ...
    if (this.productos[i].productos[y].cantidad === 0) {
      const x = this.prodsPreviewInCart.indexOf(producto);
      console.log(x);
      this.prodsPreviewInCart.splice(x, 1);
      console.log(this.prodsPreviewInCart);
  ...
  async muestraProducto(prod, pas) {
    ...
    const modal = await this.modalController.create({
      ...
      componentProps: { producto: prod, id: this.id, pasillo: pas}
    });
    modal.onDidDismiss().then((info) => {
      ...
      if (info.data.cambios) {
        ...
        if (info.data.eliminados.length > 0) {
          info.data.eliminados.forEach(eliminado => {
            const i = this.productos.findIndex(p => p.nombre === eliminado.pasillo);
            const y = this.productos[i].productos.findIndex(p => p.id === eliminado.id);
            if (i >= 0 && y >= 0) {
              this.productos[i].productos[y].cantidad = 0;
              this.productos[i].productos[y].total = 0;
              this.productos[i].productos[y].cart = false;
            }
          });
        }
  ...
  async muestraPasillo(type) {
    ...
    modal.onDidDismiss().then((resp) => {
      console.log(resp);
      this.modalClosed = true;
      if (resp.data.cambios) { return; }
      this.productos.forEach(async (p, i) => {
        this.productos[i].productos = await this.pasilloService.updateProductsQty(Object.values(p.productos));
      });
      if (resp.data.eliminados.length > 0) {
        resp.data.eliminados.forEach(eliminado => {
          const i = this.productos.findIndex(p => p.nombre === eliminado.pasillo);
          const y = this.productos[i].productos.findIndex(p => p.id === eliminado.id);
          if (i >= 0 && y >= 0) {
            this.productos[i].productos[y].cantidad = 0;
