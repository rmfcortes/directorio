...
export class FichaNegocioPage {
  ...
  async getDatos() {
    ...
    if (this.negocio.hasProductos && this.negocio.vista === 'Bloque') {
      await this.getInfoProdsBloque();
    } else if (this.negocio.hasProductos && this.negocio.vista === 'Lista') {
  ...
  async getInfoProdsLista() {
    return new Promise(async (resolve, reject) => {
      this.productosLista = await this.negocioService
        .getProductosLista(this.id, this.pasillos[this.yPasillo], this.batchLista, this.lastKey);
