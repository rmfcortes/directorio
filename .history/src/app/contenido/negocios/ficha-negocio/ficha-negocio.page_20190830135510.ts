...
export class FichaNegocioPage {
    ...
    if (prod) {
      ...
      if (this.uid) {
        prod.forEach(async (p, i) => {
          p.productos = await this.pasilloService.updateProductsQty(Object.values(p.productos), 'bloque');
        });
      } else {
        prod.forEach(async (p, i) => {
  ...
  async evaluaCantFav() {
    return new Promise(async (resolve, reject) => {
      ...
      this.productos.forEach(async (p, i) => {
        this.productos[i].productos = await this.pasilloService.updateProductsQty(Object.values(p.productos), this.negocio.vista);
