...
export class FichaNegocioPage {
  ...
  async getInfoProds() {
    return new Promise(async (resolve, reject) => {
      ...
      this.prodsInZero = prod.concat();
      this.prodsInZero.unshift(destcadosArray);
  ...
  async muestraPasillo(type) {
    ...
    modal.onDidDismiss().then((resp) => {
      ...
      this.modalClosed = true;
      if (resp.data) { return; }
      this.productos = [...this.prodsInZero];
      this.productos.forEach(async (p, i) => {
        this.productos[i].productos = await this.pasilloService.updateProductsQty(Object.values(p.productos));
      });
    });
  ...
  async loadData(event) {
    ...
    if (prod) {
      ...
      this.prodsInZero = this.prodsInZero.concat([...prod]);
      if (this.uid) {
