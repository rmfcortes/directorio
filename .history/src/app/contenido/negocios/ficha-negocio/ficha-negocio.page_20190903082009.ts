...
export class FichaNegocioPage {
  ...
  async getProds(event?) {
    return new Promise(async (resolve, reject) => {
      ...
      if (productos) {
        ...
        console.log('NO hubo productos, pero hay más pasillos');
        this.yPasillo++;
        this.lastKey = null;
        this.getProds();
      } else {
        console.log('Ni productos ni pasillos');
        this.noMore = true;
        this.prodsReady = true;
  ...
  async evaluaProdsLista(productos, event?) {
    ...
    console.log(this.batchLista);
    if (productos.length === this.batchLista + 1) {
      console.log('Hay tantos productos como batch');
      productos.pop();
      ...
      console.log('Justos batch y hay más pasillos');
      await this.agregaProductos(productos, this.pasillos[this.yPasillo], event);
      this.yPasillo++;
      this.lastKey = null;
      this.prodsReady = true;
    } else {
      console.log('Ni productos ni pasillos 2');
      this.noMore = true;
      this.prodsReady = true;
      if (event) { event.target.complete(); }
    }
    if (productos.length < this.batchLista && this.yPasillo + 1 < this.pasillos.length) {
      console.log('Faltaron productos pero hay más pasillos');
      await this.agregaProductos(productos, this.pasillos[this.yPasillo], event);
      this.lastKey = null;
      this.yPasillo++;
      this.getProds();
    } else {
      console.log('Faltaron productos pero no hay más pasillos');
