...
export class FichaNegocioPage {
  ...
  async getInfoProdsLista() {
    ...
    if (this.productos.length < this.batchLista) {
      let primerosProductos = await this.negocioService
        .getProductosLista(this.id, this.pasillos[this.yPasillo], this.batchLista + 1);
      primerosProductos = await this.pasilloService.filtraFavs(primerosProductos);
      console.log(primerosProductos);
      this.evaluaProdsLista(primerosProductos);
    } else {
      this.prodsReady = true;
    }
  }

  async evaluaProdsLista(productos) {
    if (productos.length === this.batch + 1) {
      this.lastKey = productos[productos.length - 1].nombre;
      productos.pop();
      this.agregaProductos(productos, this.pasillos[this.yPasillo]);
    } else if (productos.length === this.batch && this.yPasillo + 1 < this.pasillos.length) {
      await this.agregaProductos(productos, this.pasillos[this.yPasillo]);
      this.yPasillo++;
      this.lastKey = null;
    } else {
      this.noMore = true;
  ...
  async agregaProductos(prod, pasillo) {
    return new Promise(async (resolve, reject) => {
  ...
  async evaluaCant(productos): Promise<Producto[]> {
    return new Promise(async (resolve, reject) => {
      for (let index = 0; index < productos.length; index++) {
        productos[index].productos = await this.pasilloService.updateProductsQty(Object.values(productos[index].productos), 'vista');
        if (productos[index].productos.length === 0) {
