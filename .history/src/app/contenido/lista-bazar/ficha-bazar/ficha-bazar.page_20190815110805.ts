...
export class FichaBazarPage implements OnInit, OnDestroy {
  ...
  async publicarPregunta(comentario) {
    ...
    try {
      await this.usuarioService.publicarPregunta(pregunta, this.clasificado, this.uid);
  ...
  regresar() {
    this.modalController.dismiss({fav: this.isFavorito, foto: this.articulo.url[0]});
  }
