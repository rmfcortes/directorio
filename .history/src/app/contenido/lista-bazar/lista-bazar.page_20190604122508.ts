...
export class ListaBazarPage implements OnInit {
  ...
  getAnuncios() {
    const anunSub = this.bazarService.getAnuncios().subscribe((categorias: any) => {
      ...
      if (categorias) {
        ...
        this.anunciosRespaldo = [...this.anuncios];
  ...
  async getAnuncioCategoria(categoria) {
    const anuncios: any = await this.bazarService.getAnunciosCategoria(categoria);
  ...
  async presentCategorias() {
    ...
    this.categorias.forEach(categoria => {
      buttons.push({
        text: categoria,
        handler: () => {
          this.getAnuncioCategoria(categoria);
        }
      });
    });
    buttons.push({
