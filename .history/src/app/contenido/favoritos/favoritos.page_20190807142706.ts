...
export class FavoritosPage implements OnInit {
  ...
  async presentOferta(oferta) {
    ...
    modal.onDidDismiss().then(async (respuesta) => {
      const i = this.favoritos.findIndex(of => of.key === respuesta.data.info.key);
      this.favoritos[i].guardado = respuesta.data.info.favorito;
      if (respuesta.data.showDetail) {
        this.muestraNegocio(respuesta.data.showDetail);
      }
    });
    return await modal.present();
  }
