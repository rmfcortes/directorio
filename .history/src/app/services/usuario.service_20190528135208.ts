...
export class UsuarioService {
  ...
  async getComentarios() {
    return new Promise(async (resolve, reject) => {
      const uid = await this.getUid();
      const comentariosSub = this.db.object(`usuarios/${uid}/comentarios`).valueChanges()
        .subscribe(comentarios => {
  ...
  publicarPregunta(origen, pregunta, categoria?) {
    return new Promise(async (resolve, reject) => {
      const uid = await this.getUid();
      pregunta.id = this.db.createPushId();
      const updatedUserComent =  {};
      if (origen === 'bazar') {
        updatedUserComent[`anuncios/${origen}/${categoria}/${pregunta.idNegocio}/hasPreguntas`] = true;
      }
      updatedUserComent[`usuarios/${uid}/preguntas/${origen}/${pregunta.id}`] = pregunta;
      updatedUserComent[`preguntasAnuncios/${pregunta.idNegocio}/${uid}`] = pregunta;
      await this.db.object(`/`).update(updatedUserComent);
      resolve(true);
    });
  }
