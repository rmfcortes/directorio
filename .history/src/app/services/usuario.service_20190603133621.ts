...
export class UsuarioService {
  ...
  async guardarFavorito(negocio, categoria) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        await this.db.object(`usuarios/${uid}/favoritos/${categoria}/${negocio.id}`).set(negocio);
        resolve(true);
      } catch (error) {
        reject(error);
      }
  ...
  async getFavorito(id, categoria) {
    return new Promise(async (resolve, reject) => {
      try {
        const uid = await this.getUid();
        const favSub = this.db.object(`usuarios/${uid}/favoritos/${categoria}/${id}`).valueChanges()
  ...
  async borrarFavorito(id, categoria) {
    const uid = await this.getUid();
    return new Promise(async (resolve, reject) => {
      try {
        await this.db.object(`usuarios/${uid}/favoritos/${categoria}/${id}`).remove();
        resolve(true);
      } catch (error) {
        reject(error);
      }
