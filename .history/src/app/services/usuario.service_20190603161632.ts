...
export class UsuarioService {
  ...
  async guardarFavorito(negocio, categoria) {
    const uid = await this.getUid();
    return new Promise(async (resolve, reject) => {
      try {
        await this.db.object(`usuarios/${uid}/favoritos/${categoria}/${negocio.categoria}/${negocio.id}`).set(negocio);
        resolve(true);
      } catch (error) {
        reject(error);
      }
  ...
  async getFavorito(id, categoria, clasificado) {
    return new Promise(async (resolve, reject) => {
      try {
        const uid = await this.getUid();
        const favSub = this.db.object(`usuarios/${uid}/favoritos/${categoria}/${clasificado}/${id}`).valueChanges()
  ...
  async borrarFavorito(id, categoria, clasificado) {
    const uid = await this.getUid();
    return new Promise(async (resolve, reject) => {
      try {
        await this.db.object(`usuarios/${uid}/favoritos/${categoria}/${clasificado}/${id}`).remove();
        resolve(true);
      } catch (error) {
        reject(error);
      }
