    ...
    return new Promise((resolve, reject) => {
      if (lastKey) {
        const x = this.db.list(`solo-lectura/negocios/valoraciones/${id}/comentarios`, data =>
          data.orderByChild('puntos').limitToFirst(batch).startAt(lastKey))
            .valueChanges().subscribe((valoraciones: ComentarioVistaPrevia[]) => {
              x.unsubscribe();
              resolve(valoraciones);
            });
        ...
        const x = this.db.list(`solo-lectura/negocios/valoraciones/${id}/comentarios`, data =>
          data.orderByChild('puntos').limitToFirst(batch)).valueChanges().subscribe((valoraciones: ComentarioVistaPrevia[]) => {
            x.unsubscribe();
            resolve(valoraciones);
          });
      }
