...
export class UsuarioService {
  ...
  async generarPedido(productos, detalles, vista, cuenta) {
    const uid = await this.uidService.getUid();
    return new Promise(async (resolve, reject) => {
      try {
        ...
        updatedCountData[`usuarios/${uid}/pedidos/cuenta/${key}`] = cuenta;
        await this.db.object('/').update(updatedCountData);
        resolve(true);
      } catch (error) {
        reject(error);
  ...
  async getFavorito(id, clasificado) {
    return new Promise(async (resolve, reject) => {
      try {
        const uid = await this.uidService.getUid();
        const favSub = this.db.object(`usuarios/${uid}/favoritos/${clasificado}`).valueChanges()
  ...
  async getCuenta(uid, pedido) {
    return new Promise(async (resolve, reject) => {
      const x = this.db.object(`usuarios/${uid}/pedidos/cuenta/${pedido}`).valueChanges().subscribe((cuenta) => {
          x.unsubscribe();
          resolve(cuenta);
        });
    });
