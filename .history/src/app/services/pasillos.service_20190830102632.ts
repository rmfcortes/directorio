...
export class PasillosService {
  ...
  async guardarProductoFavorito(id, producto, nombre) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await this.db.object(`usuarios/${this.uid}/favoritos/productos/${id}/${producto.id}`).update(favorito);
        this.favs.push(fav);
        resolve(true);
      } catch (error) {
        reject(error);
  ...
  async getAllProductosFavoritos(id) {
    return new Promise(async (resolve, reject) => {
      try {
        const favSub = this.db.list(`usuarios/${this.uid}/favoritos/productos/${id}`).valueChanges()
          .subscribe(favoritos => {
            favSub.unsubscribe();
            if (favoritos) {
              this.favs = favoritos;
