...
export class PasillosService {
  ...
  async guardarProductoFavorito(id, producto) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await this.db.object(`usuarios/${this.uid}/favoritos/productos/${producto.id}`).update(producto);
        this.favs.push(fav);
        resolve(true);
      } catch (error) {
        reject(error);
  ...
  async addProductoFav(producto: Producto, id) {
    ...
    delete prodFav.cart;
    try {
      await this.guardarProductoFavorito(id, producto);
