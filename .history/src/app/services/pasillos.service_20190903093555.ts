...
export class PasillosService {
  ...
  filtraFavsBloque(pasillos: Pasillo[]): Promise<Pasillo[]> {
    return new Promise((resolve, reject) => {
      if (this.localFavs.length > 0) {
        for (const pasillo of pasillos) {
          this.localFavs.forEach((fav) => {
            console.log(pasillo);
            const index = Object.values(pasillo.productos).findIndex(p => p.id === fav.id);
            if (index >= 0) {
              pasillo.productos.splice(index, 1);
              fav.use = true;
            }
          });
  ...
  async addProductoFav(producto: Producto, id, nombre) {
    ...
    delete prodFav.cart;
    try {
      await this.guardarProductoFavorito(id, producto, nombre);
