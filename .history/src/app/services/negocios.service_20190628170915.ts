...
import { Pregunta, Pasillo, Producto, ResultNegocio,
         PrevDataNegocio, ComentarioVistaPrevia
...
export class NegociosService {
  ...
  getOpiniones(id, batch, lastKey?): Promise<ComentarioVistaPrevia[]> {
    return new Promise((resolve, reject) => {
      if (lastKey) {
        const x = this.db.list(`solo-lectura/negocios/valoraciones/${id}/comentarios`, data =>
          data.orderByKey().limitToFirst(batch).startAt(lastKey)).valueChanges().subscribe((valoraciones: ComentarioVistaPrevia[]) => {
            x.unsubscribe();
            resolve(valoraciones);
          });
      } else {
        const x = this.db.list(`solo-lectura/negocios/valoraciones/${id}/comentarios`, data =>
          data.orderByKey().limitToFirst(batch)).valueChanges().subscribe((valoraciones: ComentarioVistaPrevia[]) => {
            x.unsubscribe();
            resolve(valoraciones);
          });
      }
