...
import { VistaPreviaPedido, Status } from '../interfaces/pedido';
...
export class UsuarioService {
  ...
  getPedido(uid, pedido): Promise<Status> {
    return new Promise((resolve, reject) => {
      const x = this.db.object(`usuarios/${uid}/pedidos/detalles/${pedido}`).valueChanges().subscribe((data: Status) => {
        x.unsubscribe();
        resolve(data);
      });
    });
