...
export class PasillosService {
  ...
  async borrarProductoFavorito(id, categoria, producto) {
    return new Promise(async (resolve, reject) => {
      try {
        await this.db.object(`usuarios/${this.uid}/favoritos/productos/todos/${id}/${categoria}/${producto.id}`).remove();
        await this.db.object(`usuarios/${this.uid}/favoritos/productos/por-negocio/${id}/${producto.id}`).remove();
