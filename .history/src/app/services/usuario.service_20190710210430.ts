...
import { VistaPreviaPedido } from '../interfaces/pedido';
...
export class UsuarioService {
  ...
  async getPedidos(uid, batch, lastKey?): Promise<VistaPreviaPedido[]> {
    return new Promise(async (resolve, reject) => {
      if (lastKey) {
        const x = this.db.list(`usuarios/${uid}/pedidos/vista-previa/`, data =>
          data.orderByKey().limitToLast(batch).startAt(lastKey))
            .valueChanges().subscribe((pedidos: VistaPreviaPedido[]) => {
              x.unsubscribe();
              resolve(pedidos);
            });
      } else {
        const x = this.db.list(`usuarios/${uid}/pedidos/vista-previa/`, data =>
          data.orderByKey().limitToLast(batch)).valueChanges().subscribe((pedidos: VistaPreviaPedido[]) => {
            x.unsubscribe();
            resolve(pedidos);
          });
      }
