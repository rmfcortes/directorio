    ...
    return new Promise((resolve, reject) => {
      if (lastKey) {
        const x = this.db.list(`solo-lectura/negocios/valoraciones/${id}/comentarios`, data =>
          data.orderByChild('puntos').limitToLast(batch).endAt(lastKey))
            .valueChanges().subscribe((valoraciones: ComentarioVistaPrevia[]) => {
              x.unsubscribe();
              resolve(valoraciones);
            });
        ...
        const x = this.db.list(`solo-lectura/negocios/valoraciones/${id}/comentarios`, data =>
          data.orderByChild('puntos').limitToLast(batch)).valueChanges().subscribe((valoraciones: ComentarioVistaPrevia[]) => {
            x.unsubscribe();
            resolve(valoraciones);
          });
      }
