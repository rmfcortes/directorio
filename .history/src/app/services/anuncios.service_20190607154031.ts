...
export class AnunciosService {
  ...
  publicarNegocio(negocio) {
    return new Promise(async (resolve, reject) => {
      const uid = await this.uidService.getUid();
      await this.db.object(`usuarios/${uid}/anuncios/negocios/${negocio.id}`).update(negocio);
      resolve(true);
    });
  }
  ...
  publicarBazar(producto) {
    return new Promise(async (resolve, reject) => {
      ...
      updatedUserComent[`anuncios/bazar/${producto.id}`] = producto;
