...
export class PasillosService {
  ...
  async addProductoCarrito(uid, id, carrito, categoria, cuenta) {
    return new Promise(async (resolve, reject) => {
      try {
        const updatedCountData: any = {};
        updatedCountData[`usuarios/${uid}/carrito/productos/${id}/${categoria}`] = carrito;
        updatedCountData[`usuarios/${uid}/carrito/cuenta`] = cuenta;
        updatedCountData[`usuarios/${uid}/carrito/cantidad`] = carrito.length;
  ...
  async getCartCategoria(uid, id, categoria) {
    return new Promise(async (resolve, reject) => {
      try {
        const cartSub = this.db.object(`usuarios/${uid}/carrito/productos/${categoria}/${id}`).valueChanges()
          .subscribe(favoritos => {
            cartSub.unsubscribe();
            if (favoritos) {
              resolve(favoritos);
